...
func (uc *UserCreate) SetIDFromUserMs(i int) *UserCreate {
	uc.mutation.SetIDFromUserMs(i)
	return uc
}
...
func (uc *UserCreate) SetJoinedAt(t time.Time) *UserCreate {
	uc.mutation.SetJoinedAt(t)
	return uc
}

// SetNillableJoinedAt sets the "joined_at" field if the given value is not nil.
func (uc *UserCreate) SetNillableJoinedAt(t *time.Time) *UserCreate {
	if t != nil {
		uc.SetJoinedAt(*t)
	}
	return uc
...
func (uc *UserCreate) check() error {
	if _, ok := uc.mutation.IDFromUserMs(); !ok {
		return &ValidationError{Name: "id_from_user_ms", err: errors.New(`ent: missing required field "User.id_from_user_ms"`)}
	}
	if v, ok := uc.mutation.IDFromUserMs(); ok {
	...
	if v, ok := uc.mutation.ProfileImage(); ok {
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if value, ok := uc.mutation.IDFromUserMs(); ok {
		_spec.SetField(user.FieldIDFromUserMs, field.TypeInt, value)
		_node.IDFromUserMs = value
	}
