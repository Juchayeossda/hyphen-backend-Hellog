...
type User struct {
	...
	// IDFromUserMs holds the value of the "id_from_user_ms" field.
	IDFromUserMs int `json:"id_from_user_ms,omitempty"`
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case user.FieldID, user.FieldIDFromUserMs:
			values[i] = new(sql.NullInt64)
		case user.FieldName, user.FieldProfileImage:
			values[i] = new(sql.NullString)
		case user.FieldJoinedAt, user.FieldUpdatedAt:
	...
	for i := range columns {
		...
		case user.FieldIDFromUserMs:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field id_from_user_ms", values[i])
			} else if value.Valid {
				u.IDFromUserMs = int(value.Int64)
			}
		case user.FieldName:
...
func (u *User) String() string {
	...
	builder.WriteString("id_from_user_ms=")
	builder.WriteString(fmt.Sprintf("%v", u.IDFromUserMs))
	builder.WriteString(", ")
	...
	builder.WriteString(u.JoinedAt.Format(time.ANSIC))
	builder.WriteString(", ")
